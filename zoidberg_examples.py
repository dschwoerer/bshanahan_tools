...
def rotating_ellipse(
    ...
    field = zb.field.RotatingEllipse(
        xcentre=xcentre, I_coil=I_coil, radius=2 * a, yperiod=yperiod * 2, Btor=Btor
    )
    ...
    if rectangular:
        ...
        if curvilinear_outer_limited:
            ...
            def limit(outer):
                ...
                z0 = -0.35
                z1 = 0.02
                Z = np.maximum(Z, np.tanh((Z - z0) / z1) * z1 + z0)
                # plt.plot(Z)
                # plt.plot(outer.Z)
                outer = zb.rzline.RZline(outer.R, Z)
                return outer

            outer_lines = [limit(outer) for outer in outer_lines]
            # plt.show()
        if show_surfaces:
            for i, outer in enumerate(outer_lines):
                plt.plot(outer.R, outer.Z, "x-", label=i)
                print(
            ...
            for i, outer in enumerate(outer_lines):
                axs[0].plot(outer.R, "x-", label=i)
                axs[1].plot(outer.Z, "x-", label=i)
            axs[0].legend()
            axs[1].legend()
            plt.show()
            # raise
...
def W7X(
    ...
    """
    nx : int
        points in radial direction, excluding boundary
    ny : int
        points in toroidal direction
    nz : int
        points in poloidal direction
    fname : str
        path to write the mesh to
    npoints : int
        only used for vacuum; how many points should be used for shape evaluation.
    """
    yperiod = 2 * np.pi / 5.0
    ycoords = np.linspace(0.0, yperiod, ny, endpoint=False)
    if outer_VMEC:
...
def get_lines(
    ...
    # zb.plot.plot_poincare(
    #    field, start_r, start_z, yperiod, y_slices=yslices, revs=npoints
    # )
    rzcoord, ycoords = zb.fieldtracer.trace_poincare(
        field, start_r, start_z, yperiod, y_slices=yslices, revs=npoints
    )
    print(rzcoord.shape, ycoords.shape)
    print(len(rzcoord), len(ycoords))
    print(type(rzcoord), type(ycoords))
    lines = []
